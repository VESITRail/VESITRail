...
type BookletsTableProps = {
	...
	onFilterChange: (filters: { status?: ConcessionBookletStatusType | "all" }) => void;
};
...
const BookletsTable = ({
	...
	const handleStatusFilter = useCallback(
		(value: string) => {
			...
			onFilterChange({
				status: value === "all" ? "all" : (value as ConcessionBookletStatusType)
			});
		},
		[onFilterChange]
	);
	...
	const columns: ColumnDef<BookletItem>[] = useMemo(
		() => [
			...
			},
			{
				...
				cell: ({ row }) => (
					<div className="text-center">
						<span className="font-mono font-medium">#{row.original.bookletNumber}</span>
					</div>
				)
			},
			{
			...
			},
			{
				...
				cell: ({ row }) => (
					<div className="text-center">
						<span className="font-medium">{row.original._count.applications}</span>
					</div>
				)
			},
			{
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="end" className="w-48">
								<DropdownMenuItem asChild>
									<Link className="flex items-center" href={`/dashboard/admin/booklets/${row.original.id}`}>
	...
	const renderTableContent = () => {
		...
		if (isError) {
			return (
				<TableBody className="h-64">
					<TableRow>
						<TableCell className="text-center py-8" colSpan={table.getHeaderGroups()[0].headers.length}>
							<div className="flex flex-col items-center gap-2">
								<AlertCircle className="size-8 text-destructive" />
								<span className="text-lg font-medium">Error Loading Booklets</span>
		...
		if (sortedBooklets.length === 0) {
			return (
				<TableBody className="h-64">
					<TableRow>
						<TableCell colSpan={table.getHeaderGroups()[0].headers.length} className="text-center py-8">
							<div className="flex flex-col items-center gap-2">
			...
			<AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							{bookletToDelete?._count?.applications && bookletToDelete._count.applications > 0 ? (
								<>
									Booklet #{bookletToDelete?.bookletNumber} cannot be deleted because it has{" "}
									{bookletToDelete._count.applications} associated application
									{bookletToDelete._count.applications > 1 ? "s" : ""}.
								</>
							) : (
								<>
									Are you sure you want to delete booklet #{bookletToDelete?.bookletNumber}? This action cannot be
									undone.
								</>
							)}
						</AlertDialogDescription>
					...
					<AlertDialogFooter className="gap-4">
						...
						<AlertDialogAction
							disabled={isDeleting || (bookletToDelete?._count?.applications ?? 0) > 0}
