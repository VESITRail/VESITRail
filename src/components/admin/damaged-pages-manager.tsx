...
const DamagedPagesManager = ({
  ...
  const toInternalPageIndex = (userPageNumber: number): number =>
    userPageNumber - 1;
  ...
  const handleAddPage = useCallback(() => {
    ...
    const updatedPages = [...damagedPages, internalPageIndex].sort(
      (a, b) => a - b
    );
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            {inputError && (
              <div className="text-sm text-destructive mt-1">{inputError}</div>
            )}
          </div>
      ...
      {damagedPages.length > 0 ? (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {damagedPages.length} page{damagedPages.length !== 1 ? "s" : ""}{" "}
            marked as damaged
          </p>
        </div>
      ) : (
