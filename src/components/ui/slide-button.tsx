...
interface SlideButtonProps extends React.ComponentProps<"button">, VariantProps<typeof buttonVariants> {
	text?: string;
	loadingText?: string;
	fullWidth?: boolean;
...
const SlideButton = forwardRef<SlideButtonRef, SlideButtonProps>(
	(
		...
		const handleDragStart = useCallback(() => {
			if (slideCompleted || disabled || isSubmitting || showSubmittingState) return;
		...
		const handleDragEnd = useCallback(
			(_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
				...
				const newX = Math.max(0, Math.min(info.offset.x, dragConstraints.right));
			...
			[slideCompleted, disabled, isSubmitting, showSubmittingState, dragConstraints.right, onSlideComplete, dragX]
		);
		...
		const handleDrag = useCallback(
			...
			[slideCompleted, disabled, isSubmitting, showSubmittingState, dragConstraints.right, dragX, onSlideComplete]
		);

		const backgroundWidth = useTransform(springX, (x) => Math.min(x + 56, containerWidth - 4));
		const textOpacity = useTransform(springX, [0, 45], [1, 0]);

		if (isLoading) {
			return (
				<div className={cn("space-y-2", fullWidth && "w-full", !fullWidth && "w-64")}>
