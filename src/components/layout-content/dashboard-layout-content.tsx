...
const DashboardLayoutContent = ({ children }: { children: React.ReactNode }) => {
		...
		const checkAndRedirect = async () => {
			...
			if (isFailure(allRolesResult)) {
				toast.error("Access Verification Failed", {
					description: "Unable to verify your account access. Please try again."
				});
			...
			if (hasMultipleRoles && !isAdminPath && !isStudentPath && !isOnboardingPath) {
			...
			if (isAdminPath) {
				...
				if (!roles.student || !["Approved", "NeedsOnboarding"].includes(roles.student.status)) {
				...
				if (!roles.student || !["NeedsOnboarding", "Rejected"].includes(roles.student.status)) {
					if (roles.student?.status === "Approved") {
						router.replace("/dashboard/student");
						return;
	...
	if (showRoleSelection && allUserRoles) {
		return <RoleSelection roles={allUserRoles} userName={session.data?.user?.name || undefined} />;
	}
