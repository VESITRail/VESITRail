...
const MultiStepForm = () => {
	...
	useEffect(() => {
		if (stepsContainerRef.current && activeStepRef.current) {
			...
			const scrollLeft = activeStep.offsetLeft - container.clientWidth / 2 + activeStep.clientWidth / 2;
	...
	const renderStep = () => {
		switch (currentStep) {
			case 1:
				return <PersonalInfo errors={errors} defaultValues={formData} setFormData={setFormData} />;
			case 2:
									...
									{rejectionInfo.reason ? (
										<div className="space-y-2 mt-3">
											<div className="p-4 bg-muted/50 rounded-md border-l-4 border-muted-foreground/20">
												<div className="flex items-start gap-2">
													...
													<span className="text-sm text-foreground">{rejectionInfo.reason}</span>
												</div>
											</div>
										...
										<div className="space-y-2">
											<p className="text-sm text-muted-foreground">
												Your previous application was rejected. Please review and update your information before
												resubmitting.
											</p>
										</div>
									)}
				...
				{currentStep != totalSteps && (
					<div className="flex justify-between mt-8 gap-4">
						<Button
							...
							className={cn("flex items-center gap-2", currentStep === 1 && "invisible")}
						>
