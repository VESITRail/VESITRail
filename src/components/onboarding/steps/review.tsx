...
const Review = ({ defaultValues, setCurrentStep }: ReviewProps) => {
			...
			try {
				...
				if (result.isSuccess) {
					...
					toast.error("Review Data Not Loading", {
						description: "Unable to load your review information. Please try again."
					});
				}
			} catch (error) {
		...
		try {
			...
			if (!reviewData) {
				toast.error("Review Data Missing", {
					description: "Review data is not available. Please try refreshing the page."
				});
	...
	if (error) {
		return <ErrorComponent error={error} isRetrying={isRetrying} onRetry={() => loadReviewData(true)} />;
	}

	if (!reviewData) {
		return (
			<ErrorComponent isRetrying={isRetrying} error="No review data available" onRetry={() => loadReviewData(true)} />
		);
	}
			...
			<Card className="shadow-sm hover:shadow-md transition-shadow">
				<CardHeader className="pb-4">
					<div className="flex items-center gap-3">
						...
						<Button size="sm" variant="outline" onClick={() => setCurrentStep(1)} className="flex items-center gap-2">
			...
			<Card className="shadow-sm hover:shadow-md transition-shadow">
				<CardHeader className="pb-4">
					<div className="flex items-center gap-3">
						...
						<Button size="sm" variant="outline" onClick={() => setCurrentStep(2)} className="flex items-center gap-2">
							<Edit3 className="size-4" />
							<span className="hidden sm:inline">Edit</span>
						</Button>
					</div>
			...
			<Card className="shadow-sm hover:shadow-md transition-shadow">
				<CardHeader className="pb-4">
					<div className="flex items-center gap-3">
						...
						<Button size="sm" variant="outline" onClick={() => setCurrentStep(3)} className="flex items-center gap-2">
							<Edit3 className="size-4" />
							<span className="hidden sm:inline">Edit</span>
						</Button>
					</div>
			...
			<Card className="shadow-sm hover:shadow-md transition-shadow">
				<CardHeader className="pb-4">
					<div className="flex items-center gap-3">
						...
						<Button size="sm" variant="outline" onClick={() => setCurrentStep(4)} className="flex items-center gap-2">
							<Edit3 className="size-4" />
							<span className="hidden sm:inline">Edit</span>
						</Button>
					</div>
					...
					<AlertDialogHeader>
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<AlertDialogDescription>Are you absolutely sure you want to submit?</AlertDialogDescription>
							</div>
						</div>
					</AlertDialogHeader>
