...
const UpdateModal = ({ open, onUpdate, onIgnore, updateInfo, onOpenChange }: UpdateModalProps) => {
	...
	const { content, canExpand, isExpanded } = formatChangelog(updateInfo.changelog);
					...
					<DialogHeader>
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-3">
								<DialogTitle className="text-lg font-semibold">Update Available</DialogTitle>
								<Badge variant="secondary">{formatVersionDisplay(updateInfo.version)}</Badge>
							</div>
						</div>
						<p className="text-sm text-muted-foreground text-left">Released on {formatDate(updateInfo.publishedAt)}</p>
					</DialogHeader>
					...
					<div className="space-y-4">
						<div className="space-y-2">
							...
							<div className="max-h-40 overflow-y-auto overflow-x-hidden">
								<ChangelogRenderer content={content} className="space-y-1 text-sm text-muted-foreground" />
								{!isExpanded && canExpand && <p className="text-xs italic mt-2">...</p>}
							</div>
						</div>

						<div className="flex gap-4 justify-end">
							<Button variant="outline" disabled={updating} onClick={handleIgnore}>
								Ignore
							</Button>
							<Button disabled={updating} className="min-w-20" onClick={handleUpdate}>
								{updating ? (
									<>
		...
		<Drawer open={open} onOpenChange={onOpenChange}>
			<DrawerContent className="max-h-[85vh]">
				<DrawerHeader className="text-left">
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-3">
							<DrawerTitle className="text-lg font-semibold">Update Available</DrawerTitle>
							<Badge variant="secondary">{formatVersionDisplay(updateInfo.version)}</Badge>
						</div>
					</div>
					<p className="text-sm text-muted-foreground text-left">Released on {formatDate(updateInfo.publishedAt)}</p>
				...
				<div className="px-4 pb-6 space-y-4">
					...
					<div className="flex gap-4">
						<Button variant="outline" className="flex-1" disabled={updating} onClick={handleIgnore}>
							Ignore
						</Button>

						<Button disabled={updating} onClick={handleUpdate} className="flex-1 h-9">
							{updating ? (
								<>
