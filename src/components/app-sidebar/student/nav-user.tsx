					...
					<DropdownMenuTrigger asChild>
						...
						>
							<Avatar className="size-8 rounded-lg">
								<AvatarImage alt={data?.user.name || "Student"} src={data?.user.image || undefined} />
								<AvatarFallback className="rounded-lg">{getUserInitials("Student", data?.user.name)}</AvatarFallback>
							</Avatar>

							<div className="grid flex-1 text-left text-sm leading-tight">
								<span className="truncate font-medium">{toTitleCase(data?.user.name)}</span>
					...
					>
						<DropdownMenuLabel className="p-0 font-normal">
							<div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
								<Avatar className="size-8 rounded-lg">
									<AvatarImage src={data?.user.image || undefined} alt={toTitleCase(data?.user.name) || "Student"} />
									<AvatarFallback className="rounded-lg">{getUserInitials("Student", data?.user.name)}</AvatarFallback>
								</Avatar>
							...
							onClick={async () => {
								...
								try {
										...
										fetchOptions: {
											...
											onError: () => {
												...
												toast.error("Sign Out Failed", {
													description: "Unable to log you out right now. Please try again."
												});
									...
									toast.error("Sign Out Error", {
										description: "Something unexpected happened while logging out. Please try again."
									});
									setIsSigningOut(false);
								}
							}}
