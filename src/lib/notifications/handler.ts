...
export const sendNotification = async (payload: NotificationPayload): Promise<Result<NotificationResult, AppError>> => {
	try {
		const scenario = getNotificationScenario(payload.scenarioId);

		if (!scenario) {
			console.error("Invalid scenario ID:", payload.scenarioId);
			return failure(validationError(`Invalid scenario ID: ${payload.scenarioId}`));
		}
...
const sendEmailNotification = async (params: {
	...
	try {
		const emailTemplate = generateEmailTemplate(params.scenario, params.templateParams);
...
export const sendAddressChangeNotification = async (
	...
	return sendNotification({
		studentId,
		applicationId,
		scenarioId: isApproved ? "address_change_approval" : "address_change_rejection",
		templateParams: {
			toStation,
			fromStation,
			applicationId,
		...
		url: isApproved ? "/dashboard/student/profile" : "/dashboard/student/change-address"
	});
};
