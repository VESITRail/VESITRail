...
const Admin = () => {
	...
	const [statusFilter, setStatusFilter] = useState<ConcessionApplicationStatusType | "all">("all");
	const [typeFilter, setTypeFilter] = useState<ConcessionApplicationTypeType | "all">("all");

	const [paginationData, setPaginationData] = useState<PaginatedResult<AdminApplication>>({
		data: [],
		totalCount: 0,
		totalPages: 0,
		currentPage: 1,
		...
		<div className="py-8 px-6 lg:px-8 space-y-6">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="space-y-2">
					...
					<p className="text-muted-foreground">View and manage all concession applications submitted by students.</p>
				</div>
			</div>
