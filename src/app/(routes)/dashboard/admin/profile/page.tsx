			...
			<div className="flex justify-between items-center">
				<span className="flex items-center gap-3">
					<Avatar className="size-8 rounded-lg">
						...
						/>
						<AvatarFallback className="rounded-lg">{getUserInitials("Admin", profileData.user.name)}</AvatarFallback>
					</Avatar>
				...
				<Popover>
					...
					<PopoverContent align="end" side="bottom" className="text-sm bg-background">
						...
						<ul className="list-disc pl-5 space-y-1 text-muted-foreground">
							<li>Created: {format(new Date(profileData.createdAt), "MMM dd, yyyy 'at' h:mm a")}</li>
							<li>Last Edit: {format(new Date(profileData.updatedAt), "MMM dd, yyyy 'at' h:mm a")}</li>
						</ul>
					</PopoverContent>
				</Popover>
			</div>
					...
					<CardContent className="px-4">
						<div className="flex items-center gap-3">
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xl font-bold text-foreground">{profileData.studentsCount.toLocaleString()}</p>
							</div>
						</div>
					</CardContent>
				</Card>
					...
					<CardContent className="px-4">
						<div className="flex items-center gap-3">
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xl font-bold text-foreground">{profileData.addressChangesCount.toLocaleString()}</p>
							</div>
						</div>
					</CardContent>
				</Card>
					...
					<CardContent className="px-4">
						<div className="flex items-center gap-3">
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xl font-bold text-foreground">{profileData.applicationsCount.toLocaleString()}</p>
							</div>
						</div>
					</CardContent>
				</Card>
				...
				<CardHeader className="pb-2">
					<div className="flex flex-col gap-3">
						<div className="flex items-center gap-3">
							...
							<Badge variant={profileData.user.emailVerified ? "default" : "destructive"} className="hidden sm:flex">
						...
						<Badge
							variant={profileData.user.emailVerified ? "default" : "destructive"}
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div className="space-y-1">
							...
							<p className="font-medium">{toTitleCase(profileData.user.name) || "Admin"}</p>
						</div>
