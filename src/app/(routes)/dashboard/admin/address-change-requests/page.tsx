...
const AddressChangeRequests = () => {
	...
	const [paginationData, setPaginationData] = useState<PaginatedAddressChangeRequestsResult>({
		data: [],
		totalCount: 0,
		totalPages: 0,
		currentPage: 1,
		...
		async (
			...
			try {
				...
				const params: AddressChangeRequestPaginationParams = {
					page,
					pageSize,
					statusFilter: (statusFilter === "all" ? undefined : statusFilter) as AddressChangeStatusType,
				...
				if (result.isSuccess) {
					...
					setIsError(true);
					console.error("Failed to fetch address change requests:", result.error);
				}
			} catch (error) {
				setIsError(true);
				...
				toast.error("Failed to Load Requests", {
					description: "Unable to load address change requests. Please try again."
				});
			} finally {
				setIsLoading(false);
			}
	...
	const handleRequestUpdate = useCallback((updatedRequest: AddressChangeRequestItem) => {
	...
	useEffect(() => {
		// eslint-disable-next-line react-hooks/set-state-in-effect -- loadAddressChangeRequests is async; it sets loading/error state before awaiting the fetch, which matches React's documented data-fetching effect pattern. Safe: no state is derived synchronously from props/state outside the fetch.
		...
		<div className="py-8 px-6 lg:px-8 space-y-6">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="space-y-2">
					...
					<p className="text-muted-foreground">Review and manage student address change requests</p>
				</div>
			</div>
