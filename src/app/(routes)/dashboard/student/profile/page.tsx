			...
			<div className="flex justify-between items-center">
				<span className="flex items-center gap-3">
					<Avatar className="size-8 rounded-lg">
						<AvatarImage alt={session.data?.user.name || "Student"} src={session.data?.user.image || undefined} />
				...
				<Popover>
					...
					<PopoverContent align="end" side="bottom" className="text-sm bg-background">
						...
						<ul className="list-disc pl-5 space-y-1 text-muted-foreground">
							<li>
								Created: {profileData.createdAt && format(new Date(profileData.createdAt), "MMM dd, yyyy 'at' h:mm a")}
							</li>
							<li>
								Last Edit:{" "}
								{profileData.updatedAt && format(new Date(profileData.updatedAt), "MMM dd, yyyy 'at' h:mm a")}
							</li>
						</ul>
					</PopoverContent>
				</Popover>
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
						...
						<div className="space-y-1">
							...
							<p className="font-medium">
								{profileData.class?.branch?.name ?? "N/A"} ({profileData.class?.branch?.code ?? "N/A"})
							</p>
						</div>
						<div className="space-y-1">
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div className="space-y-1">
							...
							<p className="font-medium">
								{profileData.station?.name || "N/A"} ({profileData.station?.code || "N/A"})
							</p>
						</div>
						<div className="space-y-1">
							<div className="flex items-center gap-2">
